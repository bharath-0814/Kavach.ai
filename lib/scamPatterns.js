...
function analyzeUrls(text) {
  ...
  const urlRegex = /(https?:\/\/[^\s]+|www\.[^\s]+|[a-zA-Z0-9-]+\.(xyz|top|club|live|online|site|app|buzz|icu|vip|cc|tk|ml|ga|cf|gq|work|link|click|info)[^\s]*)/gi;
  ...
  for (const rawUrl of matches) {
    ...
    for (const tld of SUSPICIOUS_TLDS) {
      if (new RegExp(`\\.${tld}(\\/|$|\\?)`, 'i').test(lower)) {
