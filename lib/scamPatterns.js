...
const THREAT_LEXICON = [
  ...
  'cred', 'bhim', 'netbanking', 'atm', 'creditcard', 'debitcard',
...
function normalizeAndFuzzyMatch(rawText) {
  ...
  for (let token of tokens) {
    ...
    for (const threatWord of THREAT_LEXICON) {
      ...
      // Short words (3-4 chars) must be extremely strict to avoid false matching common words
      let allowedDist = 1;
      if (threatWord.length <= 3) {
        // Only allow match if length matches or leet difference
        if (Math.abs(cleanWord.length - threatWord.length) > 0) continue;
        allowedDist = 1;
...
const SCAM_RULES = [
  ...
  // 2. KYC / PAN / Aadhaar Expiry Threat
  {
    ...
    weight: 0.4,
    regex: /(kyc|pan|aadhaar|aadhar|pancard|document)\s*(update|verify|expire|expired|suspended|pending|karo|bhejo|jama|invalid|deactivate|link|block)/i
  },
  {
  ...
  },
  {
    ...
    weight: 0.45,
    regex: /(pan\s*aadhaar|aadhaar\s*pan)\s*(link|mandatory|penalty|fine|rs\.?\s*10000|inoperative|deactivated)/i
  },
  ...
  // 3. Bank Account / SIM Suspension
  {
    ...
    weight: 0.4,
    regex: /(account|khata|sim|card|atm|bijli|service|connection|yono|netbanking|access)\s*(band|block|suspend|deactivat|kat jaye|rok diya|freeze|bandh|disabled)/i
  },
  {
    id: 'SUSPENSION_PREDICATE',
    name: 'Suspension Predicate in Vernacular',
    weight: 0.35,
    regex: /(block|suspend|deactivat|band|freeze|kat)\s*(ho jayega|kar diya|karenge|hoga|hoye|hogi|jayegi|ho chuka)/i
  },
  ...
  // 4. Credential & Remote Access Harvesting
  {
    ...
    weight: 0.45,
    regex: /(otp|pin|cvv|password|passcode|mpin|tpin)\s*(share|bhejo|bataye|batao|send|enter|submit|kisi ko|verify|karein)/i
  },
  {
  ...
  // 6. Rewards, Cashback, Lottery Lure
  {
    ...
    weight: 0.4,
    regex: /(lottery|inam|reward|cashback|refund|won|winner|bonus|paisa|rupees|kbc|lucky\s*draw)\s*(claim|jeet|paaye|mubarak|received|credit|deposit|jackpot)/i
  },
  {
    id: 'FINANCIAL_LURE_VERNACULAR',
    name: 'Vernacular Winning / Reward Lure',
    weight: 0.35,
    regex: /(jeet gaye|badhai|claim kijiye|claim karein|paayein|inam mila|cashback mila|congratulations)/i
  },
  ...
  // 7. Electricity / Utility Bill Cut
  {
    ...
    weight: 0.45,
    regex: /(bijli|electricity|power|ebill|light bill|meter)\s*(unpaid|overdue|disconnect|bhare|jama kare|officer|raat|9 baje|bandh ho jayega)/i
  },
  ...
  // 8. Traffic e-Challan & Fake APK Scam
  {
    ...
    weight: 0.45,
    regex: /(traffic|echallan|e-challan|challan|mparivahan|parivahan|court\s*notice|driving\s*licence)\s*(pending|fine|court|pay\s*now|download|apk|seize)/i
  },
  ...
  // 9. India Post / Courier Delivery Scam
  {
    ...
    weight: 0.45,
    regex: /(india\s*post|post\s*office|parcel|courier|consignment|shipment|speed\s*post|package)\s*(failed|incomplete|wrong\s*address|hold|holding|hub|update\s*address|reschedule|delivery\s*fee)/i
  },
  ...
  // 11. SIM 5G Deactivation Scam
  {
    ...
    weight: 0.4,
    regex: /(sim|esim|4g\s*to\s*5g|5g\s*upgrade|trai\s*disconnection|sim\s*block)\s*(block|deactivat|port|kyc|expired)/i
  },
  ...
  // 12. Income Tax Refund Scam
  {
    ...
    weight: 0.45,
    regex: /(income\s*tax|it\s*department|tax\s*refund|it\s*refund|tds\s*refund|itr\s*refund)\s*(approved|credit|claim|pending|bank\s*account)/i
  },
...
const SUSPICIOUS_TLDS = new Set([
  ...
  'vip', 'cc', 'tk', 'ml', 'ga', 'cf', 'gq', 'work', 'link', 'click', 'info'
]);
...
const SHORTENER_DOMAINS = new Set([
  ...
  'ow.ly', 'cutt.ly', 'rb.gy', 'shorturl.at', 'page.link'
]);

/**
 * Extract and analyze URLs found in the SMS text.
 */
function analyzeUrls(text) {
  ...
  const urlRegex = /(https?:\/\/[^\s]+|bit\.ly\/[^\s]+|tinyurl\.com\/[^\s]+|t\.me\/[^\s]+|wa\.me\/[^\s]+|[a-zA-Z0-9-]+\.(?:xyz|top|club|live|online|site|app|buzz|icu|vip|apk)\b[^\s]*)/gi;
  ...
  for (const rawUrl of matches) {
    ...
    if (SHORTENER_DOMAINS.has(urlLower) || [...SHORTENER_DOMAINS].some(d => urlLower.includes(d))) {
      hasShortener = true;
      urlRisk = Math.max(urlRisk, 0.35);
    }

    if (urlLower.endsWith('.apk') || urlLower.includes('.apk?') || urlLower.includes('/apk/')) {
      hasApkDownload = true;
