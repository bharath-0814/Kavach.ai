...
/**
 * Multimodal OCR Extraction using Gemini 2.5 Flash Vision.
 * Accurately extracts SMS, WhatsApp, and banking notification text from screenshots.
 */
async function extractTextFromScreenshot(base64Data, mimeType = 'image/png') {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY not configured for screenshot OCR');
  }
    ...
    contents: [
      {
        parts: [
          ...
          {
            text: 'Extract the full SMS or chat message text from this screenshot accurately. Return ONLY the extracted message text without any extra explanation, markdown fences, or commentary.'
          }
        ]
      }
    ],
  ...
  throw new Error('Unable to extract text from screenshot');
}
